...
import com.google.android.exoplayer2.util2.GlUtil;
...
//  private final Bitmap logoBitmap;
  ...
  private GlUtil.Attribute2[] attributes;
  private GlUtil.Uniform2[] uniforms;
...
//    }
  ...
  public void initialize() {
    ...
    program.use();
    GlUtil.Attribute2[] attributes = program.getAttributes();
    for (GlUtil.Attribute2 attribute : attributes) {
...
//    GlUtil.checkGlError();
    ...
    GlUtil.Uniform2[] uniforms = checkNotNull(this.uniforms);
    GlUtil.Attribute2[] attributes = checkNotNull(this.attributes);
    for (GlUtil.Uniform2 uniform : uniforms) {
      switch (uniform.name) {
        case "tex_sampler_0":
    ...
    for (GlUtil.Attribute2 copyExternalAttribute : attributes) {
      copyExternalAttribute.bind();
    }
    for (GlUtil.Uniform2 copyExternalUniform : uniforms) {
