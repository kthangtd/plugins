...
static void* timeStatusContext = &timeStatusContext;
...
- (void)initCid:(NSURL *)url {
    NSArray<NSString *> * componenets = [url.query componentsSeparatedByString:@"&"];
    if (componenets != nil) {
        for (NSString* str in componenets) {
            if ([str hasPrefix:@"cid="]) {
                self._cid = [str stringByReplacingOccurrencesOfString:@"cid=" withString:@""];
                return;
            }
        }
...
- (instancetype)initWithPlayerItem:(AVPlayerItem*)item frameUpdater:(FLTFrameUpdater*)frameUpdater {
    ...
    AVPlayerItemMetadataOutput* metadataOutout = [[AVPlayerItemMetadataOutput alloc] initWithIdentifiers:nil];
    [metadataOutout setDelegate:self queue:dispatch_get_main_queue()];
    [_player.currentItem addOutput:metadataOutout];
...
- (void)observeValueForKeyPath:(NSString*)path
    ...
    if (context == timeStatusContext) {
        if (self.player.timeControlStatus == AVPlayerTimeControlStatusPaused
            && self.player.currentTime.value != 0) {
            if (_eventSink != nil) {
                _eventSink(@{
                    @"event": @"playbackPause",
                    @"values": @(self.player.currentTime.value * 1000 / self.player.currentTime.timescale)
                });
            }
...
//        if (oldValue as! Int == AVPlayer.TimeControlStatus.waitingToPlayAtSpecifiedRate.rawValue && newValue as! Int == AVPlayer.TimeControlStatus.playing.rawValue) {
//            let position = Int64(CMTimeGetSeconds((_player?.currentItem?.currentTime())!) * 1000)
//            _methodChannel?.invokeMethod("playbackPlay", arguments: position)
//        } else if (oldValue as! Int == AVPlayer.TimeControlStatus.playing.rawValue && newValue as! Int == AVPlayer.TimeControlStatus.paused.rawValue) {
//            let position = Int64(CMTimeGetSeconds((_player?.currentItem?.currentTime())!) * 1000)
//            _methodChannel?.invokeMethod("playbackPause", arguments: position)
//        }
//    }
        ...
        switch (item.status) {
            case AVPlayerItemStatusFailed:
                if (_eventSink != nil) {
                    ...
                    [event setObject:@"playbackError" forKey:@"event"];
                    [event setObject:[item.error localizedDescription] forKey:@"values"];
                    _eventSink(event);
...
- (void)disposeSansEventChannel {
    ...
    [[_player currentItem] removeObserver:self forKeyPath:@"loadedTimeRanges" context:timeStatusContext];
...
- (void)metadataOutput:(AVPlayerItemMetadataOutput *)output
    ...
    if (groups != nil && groups.count > 0
        ...
        NSString* metadataValue = item.stringValue;
        NSString* key = [metadataValue componentsSeparatedByString:@"_"].firstObject;
        NSMutableDictionary* map = [[NSMutableDictionary alloc] init];
        [map setObject:key forKey:@"key"];
        [map setObject:metadataValue forKey:@"meta"];
        [map setObject:[NSNumber numberWithBool:![key isEqualToString:self._cid]] forKey:@"is_ad"];
        _eventSink(@{
